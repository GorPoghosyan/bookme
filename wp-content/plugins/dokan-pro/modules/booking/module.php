...
class Module {
    ...
    public function __construct() {
        ...
        if ( method_exists( \WeDevs\Dokan\Product\Hooks::class, 'save_per_product_commission_options' ) ) {
            $product_class = \WeDevs\Dokan\Product\Hooks::class;
        } else {
          $product_class = \WeDevs\DokanPro\Products::class;
        }

        // save per product commission for bookable product
        add_action( 'woocommerce_process_product_meta_booking', array( $product_class, 'save_per_product_commission_options' ), 20 );
        ...
        add_filter( 'dokan_hide_inventory_template', [ $this, 'maybe_disable_inventory_template_for_booking' ], 10, 2 );
    }
    ...
    **/
    ...
    public function register_scripts() {
        ...
        if ( ! wp_script_is( 'jquery-tiptip', 'registered' ) ) {
            wp_register_script( 'jquery-tiptip', WC()->plugin_url() . '/assets/js/jquery-tiptip/jquery.tipTip' . $suffix . '.js', $require_dompurify ? [ 'jquery', 'dompurify' ] : [ 'jquery' ], WC()->version, true );
        }
                ...
                if ( ! isset( $person_ids[ $i ] ) ) {
                    ...
                    $person_type->set_props(
                        array(
                            ...
                            'cost'        => floatval( wc_clean( $person_cost[ $i ] ) ),
                            'block_cost'  => floatval( wc_clean( $person_block_cost[ $i ] ) ),
    ...
    /**
     * Disable inventory template for booking products
     *
     * @since 4.0.3
     *
     * @param bool $skip
     ...
     * @return bool
     */
    public function maybe_disable_inventory_template_for_booking( $skip, $post_id ) {
