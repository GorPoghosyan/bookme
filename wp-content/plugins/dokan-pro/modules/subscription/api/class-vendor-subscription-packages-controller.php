...
class Dokan_REST_Vendor_Subscription_Packages_Controller extends Dokan_REST_Vendor_Subscription_Controller {
    ...
    public function prepare_item_for_response( $item, $request ): WP_REST_Response {
        ...
        if ( in_array( 'recurring_period_interval', $fields, true ) ) {
            ...
            $data['recurring_period_interval'] = number_format_i18n( $item->get_meta( '_dokan_subscription_period_interval' ) ? $item->get_meta( '_dokan_subscription_period_interval' ) : 1 );
        }

        if ( in_array( 'recurring_period_type', $fields, true ) ) {
            $dokan_subscription_period_interval = $item->get_meta( '_dokan_subscription_period_interval' ) ? $item->get_meta( '_dokan_subscription_period_interval' ) : 1;
            $data['recurring_period_type'] = Helper::recurring_period( $item->get_meta( '_dokan_subscription_period' ), $dokan_subscription_period_interval );
        }
    ...
    public function get_item_schema(): array {
        $schema = [
            ...
            'properties' => [
                ...
                'short_description'        => [
                    'description' => __( 'Short description of the item.', 'dokan' ),
                    'type'        => 'string',
                    'context'     => [ 'view' ],
                    'readonly'    => true,
                ],
                'description'              => [
                    'description' => __( 'Description of the item.', 'dokan' ),
                    'type'        => 'string',
                    'context'     => [ 'view' ],
                    'readonly'    => true,
                ],
