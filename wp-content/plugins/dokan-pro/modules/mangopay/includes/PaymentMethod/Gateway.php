...
class Gateway extends WC_Payment_Gateway {
    ...
    public function is_available() {
        ...
        // On "Pay for Order" page, validate vendors from order instead of cart
        if ( isset( $_GET['pay_for_order'] ) ) {
            // Verify nonce for pay_for_order action
            if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'pay_for_order' ) ) {
                return false;
            }
            // Sanitize input using wp_unslash and absint
