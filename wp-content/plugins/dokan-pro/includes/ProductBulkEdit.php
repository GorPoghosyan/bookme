...
class ProductBulkEdit {
    ...
    public function __construct() {
        ...
        add_action( 'dokan_before_bulk_edit_save_single_item', [ $this, 'bulk_edit_save_product_brands' ] );
    ...
    public function bulk_edit() {
        ...
        if ( ! empty( $request_data['product_brands'] ) ) {
            $request_data['product_brands'] = array_map( 'absint', (array) $request_data['product_brands'] );
        }
    ...
    /**
     * Bulk save product brands for a product.
     *
     * Collects existing and new product brand IDs, merges them, and sets the brands for the product.
     *
     * @since 4.0.5
     *
     * @param WC_Product $product      The WooCommerce product object.
     * @param array      $request_data The request data containing product brands.
     *
     * @return void
     */
    public function bulk_edit_save_product_brands( WC_Product $product, array $request_data ): void {
        if ( empty( $request_data['product_brands'] ) ) {
            return;
        }

        // Collect product brands from the original product and the request data.
        $brand_ids    = dokan()->product->get_brand_ids( $product->get_id() );
        $added_brands = array_unique( array_merge( $brand_ids, $request_data['product_brands'] ) );

        // Set brands for the product.
        dokan()->product->save_brands( $product->get_id(), $added_brands );
    }
}
